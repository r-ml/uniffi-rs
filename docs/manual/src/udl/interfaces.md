...
To the foreign-language consumer, UniFFI object instances are designed to behave a much like
...
the `TodoList::duplicate` method must explicitly return an `Arc<TodoList>`, since UniFFI doesn't
know whether it will be returning a new object or an existing one:
...
the `self` parameter, it can signal this using the `[ByArc]` UDL attribute on the method:
